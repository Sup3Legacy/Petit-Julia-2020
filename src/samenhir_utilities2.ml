...
let pp_action_affiche fmt num t action rMap tokenTypeMap = 
	let t2 = if t = endString then "EOF" else t in
	if Tmap.mem t rMap then
		Format.fprintf "\t|%i, (%s _)" num t2
	else Format.fprintf "\t|%i, (%s _)" num t2;
	match action with
		| SUCCESS -> Format.fprintf fmt "Success"
		| SHIFT i -> Format.fprintf fmt "Shift %i" i
		| REDUCE r -> begin
			if Hashtbl.mem hash r then
				let (i,_,_) = Hashtbl.find hash r in Format.fprintf fmt "Action %i" i
			else begin
				let i = !compteurReduce in 
				compteurReduce := i +1;
				let red = Rmap.find r rMap in
				Hashtbl.add hash r (i,red);
				Format.fprintf fmt "Action %i" i
			end
		end
;;

let pp_action_table fmt action rMap tokenTypeMap =
	Format.fprintf fmt "let actionTable state listE pile newToken nextToken = match state, pile with\n";
	Imap.iter (fun i -> Tmap.iter (fun n act -> pp_action_affiche fmt i n act rMap tokenTypeMap)) action;;
	Format.fprintf fmt ";;\n\n"
;;


let pp_reduce_action fmt tokenTypeMap _ (num, (raw_prod, cons)) =
	let rec aux1 l = if l==[] then "tlP","tlE" else  
...
let pp_reduce_table fmt tokenTypeMap hash = 
	Format.fprintf fmt "let reduce num listE pile newToken nextToken = match num, listE, pile with\n";
