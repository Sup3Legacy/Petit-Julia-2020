...
let currentVenv = ref (Tmap.singleton "nothing" Nothing);;
let currentFenv = ref (Tmap.singleton "div" [[Int64; Int64], Int64]);;
let currentSenv = ref (Tmap.empty);;
let currentAenv = ref (Tmap.empty);;
    ...
    then Nothing
      ...
      if Tmap.mem ident fE then
        ...
        let rec aux l1 l2 = match l1,l2 with
          ...
          | _, _ -> false
        in
        let tSet = List.fold_left (fun s (pL,pjT) -> if aux pL eL then TypeSet.add pjT s else s) TypeSet.empty l in
        if TypeSet.cardinal tSet > 1
        then Any
        else
          if TypeSet.cardinal tSet = 1
          then TypeSet.choose tSet
...
and testTypBlocF2 vE fE sE aE pjT = function
  ...
  | Eapplication (_,ident,eL) ->
    ...
    if Tmap.mem ident fE then
      ...
      in
      let tSet = List.fold_left (fun s (pL,pjT) -> if aux pL eL then TypeSet.add pjT s else s) TypeSet.empty l in
      if TypeSet.cardinal tSet > 1 then Any
      else if TypeSet.cardinal tSet = 1 then TypeSet.choose tSet
        else error ("no compatible function")
    else if Tmap.mem ident sE then S ident else error ("undeclared function "^ident)
...
let verificationType declL =
  ...
  let vE, fE, sE, aE = parcours1 !currentVenv !currentFenv !currentSenv !currentAenv dl in
  let vEp, fEp, sEp, aEp = parcours2 vE fE sE aE dl in
  currentVenv := vEp;
  currentFenv := fEp;
  currentSenv := sEp;
  currentAenv := aEp;
