...
let pp_header fmt = 
	...
	Format.fprintf fmt "exception Output of rulesType list\n";
	Format.fprintf fmt "exception Samenhir_Parsing_Error\n\n\n"
...
let pp_action fmt pos t a rMap tokenTypeMap = 
	if Tmap.mem t tokenTypeMap then begin
		Format.fprintf fmt "\t| Some (%s data) -> \n" t;
		 match a with
			| SUCCESS -> Format.fprintf fmt "\t\traise (Output pileMem)"
			| SHIFT i -> Format.fprintf fmt "\t\taction%i (%i::pileEtats) (TOKEN (%s data)::pileMem) None newToken" i pos t
			| REDUCE ((n,p) as r) -> begin
				pp_reduce fmt (Rmap.find r rMap);
				Format.fprintf fmt "\t\tgoto (List.hd pileEtats) ((%s valeur)::pileMem) \"%s\" (Some (%s data)) newToken" n n t
		...
		match a with 
			| SUCCESS -> Format.fprintf fmt "\t\traise (Output pileMem)"
			| SHIFT i -> Format.fprintf fmt "\t\taction%i (%i::pileEtats) (TOKEN %s::pileMem) None newToken" i pos t
			| REDUCE ((n,p) as r) -> begin
				pp_reduce fmt (Rmap.find r rMap);
				Format.fprintf fmt "\t\tgoto (List.hd pileEtats) pileEtats ((%s valeur)::pileMem) \"%s\" (Some %s) newToken" n n t
				end 
		end;
	Format.fprintf fmt "\n"
...
let pp_actionStates fmt i actionT ruleMap tokenTypeMap = 
	...
	Format.fprintf fmt (if Tmap.mem endString actionT then "raise (Output pileMem)" else "Not_a_token");
	Format.fprintf fmt "\n";
	...
	Format.fprintf fmt "\t| _ -> raise Samenhir_Parsing_Error\n"
...
let pp_buildProg fmt program = 
	...
	Format.fprintf fmt "\t|_,_ -> raise Samenhir_Parsing_Error\n";
...
let table = buildTable (unrawGrammar g_raw) [] in 
let p = {gR = g_raw; startLTable = 0; gotoTab = table.goto; actionTab = table.action; tokenList = ["PARG", None; "PARD", None; "ADD", None; "TIMES", None;("EOF", None); "INT",Some "int"; endString, None]} in
let out = open_out "test.ml" in
